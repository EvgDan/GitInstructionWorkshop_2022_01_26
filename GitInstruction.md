...
Работа в локальном репозитории подразумевает возможность создания нескольких версий одного и того же файла.
Это может быть полезно как при работе 1 человека (создание неких черновиков), так и при одновременной работе нескольких людей.
Ветка - отклонение от основной версии файла для использования в качестве черновика/выполнения локальной задачи/проверки выполненного задания сторонним человеком и пр.

Для того, чтобы проверить наличие всех созданных веток в локальном репозитории необходимо воспользоваться командой:
...
Для перехода к новой созданной ветке и начала работы в ней необходимо воспользоваться командой:

> git checkout branch_name

После завершения работы в данной ветке, при необходимости возможно ее слияение с основной версией файла. Для этого необходимо переместиться в основную версию файла (ветку master) и выполнить команду по слияние версий:

> git merge branch_name

Даннная команда вливает все содержимое ветки branch_name в основную версию файла (ветку master)

При необходимости ветку можно удалить, воспользовавшись командами:

> git branch -d branch_name (для уже слитых веток)

> git branch -D branch_name (для всех веток)
